...
class DCClockSettingViewController: LXMBaseViewController {
    ...
    class func loadFromStroyboardWithTargetAlarm(_ alarm: DCAlarm?) -> DCClockSettingViewController {
        let viewController = DCClockSettingViewController.swift_loadFromStoryboard("ClockRefe")
        if alarm == nil {
...
extension DCClockSettingViewController {
    ...
    func setupNavigationBar() {
        ...
        let cancell = UIBarButtonItem(title: "取消", style: .plain, target: self, action:  #selector(DCClockSettingViewController.handleCancelButtonTapped(_:)))
        
        let confirm = UIBarButtonItem(title: "存储", style: .plain, target: self, action:  #selector(DCClockSettingViewController.handleConfirmButtonTapped(_:)))
